...
class LIVE_PLOT_APP(QtWidgets.QMainWindow):
    ...
    def Two_Layer_Parameters(self):
        self.x_values = []
        # Obtener los valores iniciales de los campos de entrada
        try:
            rho_01 = float(self.ui.line_rho_01.text())
            rho_02 = float(self.ui.line_rho_02.text())
            h_0 = float(self.ui.line_h_0.text())
        except ValueError:
            # Manejar el caso en que los valores no son números válidos
            QtWidgets.QMessageBox.warning(self, "Entrada inválida", "Por favor, ingrese valores numéricos válidos para rho_01, rho_02 y h_0.")
            return

        initial_guess = [rho_01, rho_02, h_0]
        method = self.ui.comboBox.currentText()  # Obtener el método seleccionado del comboBox
        RES_0 = minimize(self.Error_ro1, initial_guess, (self.X_a, self.Y_a), method=method, callback=self.capture)
        print("Respuesta: \n", RES_0)
        print('----------------')
        ...
        self.label_fun.setText(f"{RES_0.fun * 100:.3f} %")
    ...
    def create_plots(self):
        ...
        fig1, axs = plt.subplots(3, 1, figsize=(3, 5))
        ...
        layout1 = self.ui.widget_it.layout()
        if layout1 is None:
        ...
        layout2 = self.ui.widget_res.layout()
        if layout2 is None:
